...
Next, we'll use the `gtsummary` package, an extension of `gt` that can incorporate model outputs into tables.
...
We don't specify a test, `add_p()` will use the [default tests](The default test used in `add_p()` primarily depends on these factors:), which depend on whether the data is continuous or categorical, how many categories, etc.
...
### Table with regression results [STILL DRAFTING]

Predict chlorophyll from water temperature, nitrogen, phosphorus, and dissolved organic carbon.

```{r}
mod1 <- lm(chlorophyll_a_ugL ~ h2o_temp_c + total_nitrogen_mgL + total_phosphorus._mgL + dissolved_organic_carbon_mgL, data = data)
```
...
Use a [mix of `gt` and `gtsummary` functions for formatting](https://www.danieldsjoberg.com/gtsummary/articles/tbl_regression.html#customize-output) the output table.

Combine with `gt` formatting by making the table (currently a tbl_regression object) into a gt_tbl object, to do other gt formatting

```{r}
tbl_regression(mod1,
               #update variable names
               label = list(h2o_temp_c = "Water Temperature (C)", total_nitrogen_mgL = "Total Nitrogen (mgL)", total_phosphorus._mgL = "Total Phosphorus (mgL)", dissolved_organic_carbon_mgL = "Dissolved Organic Carbon (mgL)")) |>
  #make gt object
   as_gt() |>
  #add source note
